...
<head>
  ...
  <title>CebuTra - Reset Password</title>
...
<body class="flex justify-center items-center min-h-screen bg-gray-100 p-0 sm:p-4 font-['Plus_Jakarta_Sans']">
  ...
  <div class="w-full max-w-[412px] bg-[#FFFBF3] h-screen sm:h-[720px] shadow-2xl relative flex flex-col justify-between overflow-hidden sm:rounded-[36px] p-5 pt-2.5">
    ...
    <!-- Main Content -->
    <div class="flex-1 flex flex-col justify-between my-auto py-4">
      ...
      <div class="text-center my-auto">
        ...
        <h1 class="text-xl font-extrabold text-[#333]">Reset Password</h1>
        <p class="text-xs text-gray-500 font-medium mt-1 px-4 leading-tight">
          Enter your registered email address and we'll send you a password reset link.
        </p>
      </div>

      <!-- フォームカード（余白最適化） -->
      <div class="bg-white/80 backdrop-blur-md rounded-2xl p-4 shadow-sm border border-orange-50/50 mt-3">
        ...
        <form action="{{ route('password.email') }}" method="POST" class="space-y-3">
          ...
          <div>
            ...
            <div class="relative">
              ...
              <input type="email" name="email" value="{{ old('email') }}" placeholder="example@cebutra.com" class="w-full bg-gray-50 border border-gray-100 rounded-xl py-2.5 pl-9 pr-3 text-xs focus:outline-none focus:border-[#008080] focus:bg-white transition-all text-gray-800 font-medium" required>
            </div>
          </div>

          <button type="submit" class="w-full bg-[#FF6347] hover:bg-[#e55338] text-white font-bold py-3 rounded-xl shadow-md shadow-orange-500/20 transition-all text-xs cursor-pointer active:scale-[0.99] mt-1">
            Send Reset Link
          </button>
        </form>
      </div>
