...
<body class="flex justify-center items-center min-h-screen bg-gray-100 p-0 sm:p-4">
  ...
  <div class="w-full max-w-[412px] bg-[#FFFBF3] h-screen sm:h-[720px] shadow-2xl relative flex flex-col justify-between overflow-hidden sm:rounded-[36px]">
    ...
    <!-- メインコンテンツ領域（フォントと本文サイズを大きく調整） -->
    <div class="flex-1 overflow-y-auto no-scrollbar p-5 space-y-6 pb-28">

      <!-- タイトル ＆ 最終更新日 -->
      <div class="bg-white rounded-3xl p-5 shadow-sm border border-gray-100/50 space-y-3">
        <div class="flex items-center gap-2.5 text-[#008080]">
          <i class="fa-solid fa-shield-halved text-xl"></i>
          <span class="text-xs font-bold uppercase tracking-wider">CebuTra Policies</span>
        </div>
        <p class="text-xs text-gray-400 font-medium">{{ __('messages.terms.last_updated') }}</p>
        <p class="text-xs text-slate-600 leading-relaxed pt-1 font-medium">
          {{ __('messages.terms.intro') }}
        </p>
      </div>

      <!-- 各条項リスト -->
      <div class="bg-white rounded-3xl p-5 shadow-sm border border-gray-100/50 space-y-6">

        <!-- 第1条 -->
        <div class="space-y-2">
          <h2 class="text-sm font-bold text-slate-800 flex items-center gap-2">
            <span class="w-2 h-2 rounded-full bg-[#008080]"></span>
            {{ __('messages.terms.sec1_title') }}
          </h2>
          <p class="text-xs text-slate-600 leading-relaxed pl-4 font-medium">
        ...
        <!-- 第3条（重要） -->
        <div class="space-y-2 bg-amber-50/60 p-4 rounded-2xl border border-amber-100">
          <h2 class="text-sm font-bold text-amber-900 flex items-center gap-2">
            <i class="fa-solid fa-triangle-exclamation text-amber-500"></i>
            {{ __('messages.terms.sec3_title') }}
          </h2>
          <p class="text-xs text-amber-800/90 leading-relaxed pl-6 font-medium">
      ...
      <!-- サポート問合せリンク -->
      <div class="text-center pt-2 pb-4 space-y-2.5">
        <p class="text-xs text-gray-400 font-medium">{{ __('messages.terms.agree_contact') }}</p>
        <a href="mailto:support@cebutra.com" class="inline-flex items-center gap-2 text-xs font-bold text-[#008080] bg-white px-4 py-3 rounded-2xl border border-teal-100 shadow-sm hover:bg-teal-50 transition-colors">
