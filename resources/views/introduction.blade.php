...
@section('title', 'Welcome to CebuTra')

@section('main-class', 'flex-1 my-6 bg-white rounded-3xl p-5 border border-gray-100/80 shadow-inner overflow-y-auto space-y-4 text-gray-600 text-xs leading-relaxed')

@section('header')
<div class="text-center space-y-4 mt-16 flex-shrink-0">
...
@section('footer')
<div class="p-6 pt-0 space-y-4 flex-shrink-0 bg-[#FFFBF3] sm:rounded-b-[40px] z-20">
  <label class="flex items-start gap-3 px-2 cursor-pointer group select-none">
    <div class="relative flex items-center mt-0.5">
      <input type="checkbox" id="agreeCheckbox" onchange="toggleButton()" class="peer appearance-none w-5 h-5 border border-gray-300 rounded-lg bg-white checked:bg-[#008080] checked:border-[#008080] transition-all cursor-pointer">
      <i class="fa-solid fa-check absolute text-white text-[10px] left-1.5 opacity-0 peer-checked:opacity-100 transition-opacity pointer-events-none"></i>
    </div>
  ...
  <button id="submitBtn" onclick="navigateToNext()" disabled class="w-full bg-gray-300 text-gray-500 font-bold py-4 px-4 rounded-2xl text-sm transition-all shadow-md cursor-not-allowed">
    Accept & Continue
  </button>
</div>
@endsection
...
<script>
  function toggleButton() {
    ...
    if (checkbox.checked) {
      ...
      btn.classList.add('bg-[#FF6347]', 'text-white', 'hover:bg-[#E05338]', 'active:scale-[0.99]', 'shadow-orange-500/20');
    } else {
      btn.disabled = true;
      btn.classList.remove('bg-[#FF6347]', 'text-white', 'hover:bg-[#E05338]', 'active:scale-[0.99]', 'shadow-orange-500/20');
